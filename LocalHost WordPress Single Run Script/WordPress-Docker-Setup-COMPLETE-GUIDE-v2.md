...
By default, Windows prevents running PowerShell scripts for security reasons. You'll see this error:
...
You have **three options** to run the setup script:

#### **Option 1: Bypass for Single Script Execution (Recommended)**

Run this command each time you want to execute the script:
...
# Run with bypass
powershell -ExecutionPolicy Bypass -File .\WordPress-Docker-Setup-PRODUCTION-READY.ps1
```

**Pros:** 
- ✅ Most secure (only bypasses for this one script)
- ✅ No permanent changes to system
- ✅ No admin rights required

**Cons:**
- ⚠️ Must use this command every time

---

#### **Option 2: Temporarily Set Unrestricted (Current Session Only)**
...
# Now run script normally
.\WordPress-Docker-Setup-PRODUCTION-READY.ps1
```

**Pros:**
- ✅ Policy reverts when you close PowerShell
- ✅ Can run script normally after setting

**Cons:**
- ⚠️ Requires Administrator privileges
- ⚠️ Resets when PowerShell is closed

---

#### **Option 3: Permanently Set for Current User (Not Recommended for Security)**

```powershell
# Open PowerShell as Administrator

# Set policy permanently for current user
...
- ✅ Can run scripts normally

**Cons:**
- ⚠️ Requires Administrator privileges
- ⚠️ Less secure (allows all local scripts)
- ⚠️ Permanent system change

---

#### **Option 4: Unblock Downloaded Script**

If the script was downloaded from the internet, Windows may block it:
...
Unblock-File -Path .\WordPress-Docker-Setup-PRODUCTION-READY.ps1

# Now run normally (if execution policy allows)
.\WordPress-Docker-Setup-PRODUCTION-READY.ps1
```
...
**For most users**, use **Option 1** (Bypass for Single Execution):
...
This is the **safest** method that doesn't require admin rights or permanent system changes.
...
**Method A: With Bypass (Recommended)**
```powershell
...
3. **Unblock Downloaded File** (Important!)
   ```powershell
   # Unblock the script
   Unblock-File -Path .\WordPress-Docker-Setup-PRODUCTION-READY.ps1
   ```
...
**Alternative if you set execution policy:**
```powershell
.\WordPress-Docker-Setup-PRODUCTION-READY.ps1
```
...
**Important**: You may need to bypass execution policy for management commands too:
...
**Full Error Message:**
```
File WordPress-Docker-Setup-PRODUCTION-READY.ps1 cannot be loaded because running scripts 
...
#### Error: "Script is digitally signed but cannot be validated"
...
```powershell
# Run backup (with execution policy bypass)
...
4. Action: Start a program
   ...
   - Arguments: `-ExecutionPolicy Bypass -File "C:\path\to\wordpress-docker\scripts\deploy.ps1" -Backup`
...
# Run script with bypass (recommended)
...
*This guide covers the complete setup, configuration, and optimization of a production-ready WordPress environment using Docker on Windows, including PowerShell execution policy handling. For questions or issues, review the Troubleshooting section or consult the official documentation links.*
